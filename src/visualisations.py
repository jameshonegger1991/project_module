...
def save_individual_histograms(df, title = "Histogram"):
    """
    Save histograms for all numeric columns to 'plots/EDA/whole_dataset/univariate_plots/histograms'.
    """
    if df.empty:
...
def save_individual_barplots(df, title = "Bar Plot"):
    """
    Generate bar plots for all categorical features and save them to 'outputs/plots/EDA/whole_dataset/univariate_plots/barplots/'.
    """
    if df.empty:
...
def save_combined_histograms(df, title = "Histograms - All Numeric Features (Before Cleaning And Imputation)"):
    """
    Save a combined grid of histograms for all numeric columns to ouputs/plots/EDA/whole_dataset/univariate_plots/histograms.
    """ 
    if df.empty:
        print("The DataFrame is empty. Cannot generate histograms.")
        return
...
def save_combined_violin_plots(df, title = "Violin Plots - All Numeric Features (Before Cleaning And Imputation)"):
    """
    Save a combined grid of violin plots for all numeric columns to 'outputs/plots/EDA/whole_dataset/univariate_plots/violin_plots/'.
...
def save_combined_barplots(df, title = "Bar Plots - All Categorical Features (Before Cleaning And Imputation)"):
    """
    Save a combined grid of bar plots for all categorical columns in the DataFrame to 'outputs/plots/EDA/whole_dataset/univariate_plots/barplots/'.
    """
    if df.empty:
        print("The DataFrame is empty. Cannot generate bar plots.")
        return
...
def save_spearman_correlation_matrix(df, title = 'Spearman Correlation Matrix'):
    """
    Save the Spearman correlation matrix for all numeric variables to 'outputs/plots/EDA/train_set/multivariate_plots/'.
    """
    if df.empty:
...
def save_multiple_metrics_vs_features_plot(results_df, task, metrics=None, figsize=(14, 10)):
    """
    Plot multiple metrics into one figure with different subplots that show all models vs. number of features.
    It takes in argument the results_df generated by evaluate_k_values() and saves the plot to 'outputs/plots/model_training'.
    """
    if metrics is None:
        if task == 'regression':
...
def save_residuals_plot_from_all_results(all_results_regression, k_chosen, figsize=(14, 10)):
    ...
    (Custom wrapper designed for pipeline consistency over sklearn's PredictionErrorDisplay). The plot
    is saved to 'outputs/plots/model_training'.
    """
    os.makedirs(f"{PLOTS_DIR}/model_training", exist_ok=True)
...
def save_confusion_matrices_plot_all_models(all_results_classification, k_chosen, class_labels, figsize=(14, 10)):
    ...
    The plot is saved to 'outputs/models/model_training'.
...
def save_learning_curves_plot_all_models(all_results, k_chosen, X_train_after_var_thresh, y_train, task='regression', figsize=(14, 10)):
    """
    Plot learning curves for all models built with k-top features. The result is saved to 'outputs/plots/model_training'.
...
def barplot_global_shap_rankings(global_mean_absolute_shap_rankings_dict, top_k=10):
    """
    Plot and save the global SHAP feature importance rankings for all models 
    in the Rashomon set 'outputs/plots/SHAP_analysis'.

    Arguments:
    - global_mean_absolute_shap_rankings_dict (dict): A dictionary containing 
    the global SHAP feature importance rankings for each model, returned by the local_and_global_shap_values_calculator().
    - top_k (int, optional): The number of top features to display for each model. Defaults to 10.
    """
...
def shap_summary_plot(shap_values, feature_names):
    """
    Plot and save SHAP summary plots for all models in the Rashomon set to 'outputs/plots/SHAP_analysis'.

    Arguments:
    - shap_values (dict): A dictionary containing the local SHAP values and the corresponding 
    test data for each model, returned by run_complete_shap_analysis_and_classification().
    - feature_names (list): A list containing the feature names, returned by run_complete_shap_analysis_and_classification().
    """
