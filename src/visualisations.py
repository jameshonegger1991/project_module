...
def plot_multiple_metrics_vs_features(results_df, task, metrics=None, save_dir=None, figsize=(14, 10)):
    """
    ...
    if save_dir:
        save_path = os.path.join(save_dir, f"{task}_metrics_vs_features_plot.png")
        plt.savefig(save_path, dpi=300, bbox_inches='tight') #for better margins
        print(f"Metrics vs features plot saved to: {save_path}")
    
    plt.show()

def plot_residuals_from_all_results(all_results, k_chosen, figsize=(14, 10)):
    """
    Plots residual plots for all models at the chosen 'k' by using the 'all_results' dictionary generated by evaluate_k_values.
    """
